...
const Layout = ({ children }) => {
  ...
  const fetchResults = async () => {
    ...
    try {
      ...
      if (searchType === "save data") {
        const validSaves = data.filter(save => save && save.gameID && save.title);
  ...
  const handleSearch = () => {
    let typeParam = searchType === "save data" ? "s" : searchType === "games" ? "g" : "u";
    const sinpt = searchInput;
    setSearchInput("");
    setOptions([]);
          ...
          <div className="center d-flex align-items-center">
            <InputGroup style={{ width: '100%', maxWidth: '1200px', gap: '0.5rem' }}>
              <Typeahead
                ...
                onChange={(selected) => {
                  if (selected.length > 0) {
                    ...
                    if (searchType === "games") navigate(`/g/${option.slug}`);
                    else if (searchType === "save data") navigate(`/s/${option.id}`);
                    else if (searchType === "users") navigate(`/u/${option.label}`);
                    setOptions([]);
                    setSearchInput("");
                  }
                }}
                ...
                filterBy={() => true}
                selected={searchInput && options.length > 0 ? [{ label: searchInput }] : []}
                renderMenuItemChildren={(option) => {
                  if (searchType === "save data") {
                    return (
                      <div title={`${option.label} - ${option.gameTitle} [${option.platformAbbr}]`}>
                  ...
                  if (searchType === "games") {
                    return (
                      <div title={`${option.label}${option.platforms?.length ? " [" + option.platforms.map(p => p.abbreviation).join(", ") + "]" : ""}`}>
                        <strong>{option.label}</strong>{" "}
                  ...
                  if (searchType === "users") {
                    return (
                      <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", width: "100%" }}
                        ...
                        <img src={`${config.api.assets}/user/${option.id}/pfp?${Date.now()}`} alt={`@${option.label}'s profile`}
                          style={{ width: 30, height: 30, borderRadius: "50%", objectFit: "cover", flexShrink: 0 }}
                          onError={(e) => { e.target.onerror = null; e.target.src = `${config.api.assets}/defaults/pfp`; }} />
                        <div style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", flexGrow: 1 }}>
                          {option.alias ? <><strong>{option.alias}</strong> — @{option.label}</> : <><strong>@{option.label}</strong></>}
                        </div>
              ...
              <div className="d-flex align-items-center">
                <Form.Check inline type="radio" label="Save Data" name="searchType" value="save data"
                  checked={searchType === "save data"} onChange={(e) => setSearchType(e.target.value)} />
                <Form.Check inline type="radio" label="Games" name="searchType" value="games"
                  checked={searchType === "games"} onChange={(e) => setSearchType(e.target.value)} />
                <Form.Check inline type="radio" label="Users" name="searchType" value="users"
                  checked={searchType === "users"} onChange={(e) => setSearchType(e.target.value)} />
              </div>
              <Button variant="primary" onClick={handleSearch} style={{ flexShrink: 0 }}>Search</Button>
            </InputGroup>
          </div>
              ...
              <>
                ...
                <div className="dropdown">
                  <button className="btn btn-secondary dropdown-toggle" type="button" id="userDropdown" data-bs-toggle="dropdown" aria-expanded="false">
                    <img src={`${config.api.assets}/user/${loggedUser.userID}/pfp?${Date.now()}`} alt="Profile"
                      className="rounded-circle" style={{ width: "30px", height: "30px", objectFit: "cover", marginRight: "10px" }}
                      onError={(e) => { e.target.onerror = null; e.target.src = `${config.api.assets}/defaults/pfp`; }} />
